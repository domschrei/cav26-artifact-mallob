...
* `table-incsat.txt`, `incsat-cdf(-logscale).pdf` : The parallel variants **do not necessarily** outperform the sequential variants (especially in terms of individual solved queries), reason being that the overhead of parallel execution is often not worthwhile for many of the considered inputs. The variants with proof checking ("rtcheck") are expected to perform worse than the unchecked variants.
